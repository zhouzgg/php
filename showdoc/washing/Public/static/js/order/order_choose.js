...
layui.use(['jquery', 'layer', 'common', 'form', 'element', 'tree', 'laydate'], function () {
    ...
    $(".choose-enter").click(function () {
        /*手机表单数据*/
        var store_data = "";
        $("input[name='choose_storehouse[]']:checked").map(function(i,v){
            store_data += $(v).val() + ",";
        });
        if (store_data === "") {
            var alert = layer.alert('亲，您还没有选择库房', {
                    icon: 2,
                    skin: 'layer-ext-moon' //该皮肤由layer.seaning.com友情扩展。关于皮肤的扩展规则，去这里查阅
                }, function () {
                    layer.close(alert);
            ...
            //var store_house_title = store_house.attr("title");
            //$(".layui-breadcrumb a:eq(3) cite").html(store_house_title);
            /*日期控件隐藏的表单不能为空*/
            /*if( objFormData.start_time !== ""&&objFormData.end_time !== "") {
                ...
                three.find('.timeline-container').removeClass("hide");
                /!*隐藏的表单*!/
                objFormData.storehouse_id = store_house.val();
                objFormData.storehouse_name = store_house_title;
                // 收集所有的数据传输到下一个页面
            ...
            objFormData.storehouse_id = store_data;
            // 隐藏自身
            three.find('.time-num').removeClass("active").html("&#xe618");
            three.find('.timeline-container').addClass("hide");

            fourth.find('.time-num').addClass("active");
            fourth.find('.timeline-container').removeClass("hide");
        }
    });

    /*第4步*/
    $(".choose-end").click(function () {
        var washing_id = $(this).siblings(".layui-row").find("input[type=radio]:checked");

        if (washing_id.length === 0) {
            var alert = layer.alert('亲，您还没有选择洗涤厂', {
                    icon: 2,
                    skin: 'layer-ext-moon' //该皮肤由layer.seaning.com友情扩展。关于皮肤的扩展规则，去这里查阅
                }, function () {
                    layer.close(alert);
            ...
            objFormData.washing_id = washing_id.val();
            if( objFormData.start_time !== ""&&objFormData.end_time !== "")
            {
                // 收集所有的数据传输到下一个页面
                var type = base.getUrl().type;
    ...
    function targetUrl(type) {
        window.location.href = '/index.php/Home/Order/' + type + '_order?customer_id=' +
            objFormData.customer_id + '&department_id=' +
            objFormData.department_id + '&storehouse_id=' +
            objFormData.storehouse_id + '&washing_id=' +
            objFormData.washing_id + '&start_time=' +
