...
<div class="layui-layout layui-layout-admin">
 ...
 foreach($child['children'] as $tchild) { $childSelect = ""; if($tchild['id'] == $childNode){ $childSelect = "layui-this"; } ?>
    ...
    <div class="layui-body">
        <!-- 内容主体区域 -->
        <div style="padding: 15px;overflow-x: auto;height: 100%">
            <blockquote class="layui-elem-quote">
                <?php echo ($title); ?>
                <a href="<?php echo ($url); ?>" target="_top" class="layui-btn output-data"><?php echo (L("_EXPORT_EXCEL_TABLE_")); ?></a>
            </blockquote>
            <!--每个页面为了保持样式不冲突必须 唯一的样式必须统一一个不一样的类名-->
            <div class="layui-body-container order">
                <table class="layui-table" lay-skin="line">
                    <tbody>
                    <tr>
                        <td><?php echo (L("_ORDER_CUSTOMER_")); ?>：<?php echo ($other["customer"]); ?></td>
                        <td><?php echo (L("_ORDER_DEPARTMENT_")); ?>：<?php echo ($other["department"]); ?></td>
                    </tr>
                    <tr>
                        <td colspan=2><?php echo (L("_ORDER_TIME_")); ?>：<?php echo ($other["start_time"]); ?>~<?php echo ($other["end_time"]); ?></td>
                    </tr>
                    </tbody>
                </table>
               ...
               <table class="layui-table">
                    <thead>
                    <?php if(!empty($date)): ?><tr>
                        <th></th>
                        <?php if(is_array($date)): $i = 0; $__LIST__ = $date;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><th><?php echo ($v); ?></th><?php endforeach; endif; else: echo "" ;endif; ?>
                        <th>小计</th>
                    </tr><?php endif; ?>
                    </thead>
                    <tbody>
                    <!--数据渲染-->
                     <?php if(!empty($list)): if(is_array($list)): $key = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($key % 2 );++$key;?><tr>
                                <td><?php echo ($key); ?></td>
                                <?php if(is_array($date)): $i = 0; $__LIST__ = $date;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><!-- <td><?php echo ($val[$v]); ?></td> -->
                                    <td>
                                        <table>
                                            <tr>
                                                <td>淨</td>
                                            ...
                                            <tr>
                                                <td><?php echo ($val[$v]['SENDER_ID']); ?></td>
                                                <td><?php echo ($val[$v]['RECEIVER_ID']); ?></td>
                                            </tr>
                                        </table>
                                ...
                                <td>
                                    ...
                                    <tr>
                                        <td><?php echo ($val['合计']['SENDER_ID']); ?></td>
                                        <td><?php echo ($val['合计']['RECEIVER_ID']); ?></td>
                                    </tr>
                                </table>
                            </tr><?php endforeach; endif; else: echo "" ;endif; ?>
                        <?php else: ?>
                        <tr>
                            <td colspan="<?php echo ($i+2); ?>"> <p class="layui-word-aux">
...
<!--按每个页面引入-->
<script src="/Public/static/js/order.js"></script>
</html>