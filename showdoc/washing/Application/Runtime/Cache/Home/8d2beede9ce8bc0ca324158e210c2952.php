    ...
    <div class="layui-body">
        ...
        <div style="padding: 15px;">
            <blockquote class="layui-elem-quote">
                <?php echo ($title); ?>
                <a target=_top href="<?php echo U('User/export_user_list');?>?name=<?php $_GET['name'];?>&role_id=<?php $_GET['role_id'];?>" class="layui-btn output-data"><?php echo (L("_EXPORT_EXCEL_TABLE_")); ?></a>
            </blockquote>
            <!--每个页面为了保持样式不冲突必须 唯一的样式必须统一一个不一样的类名-->
            <div class="layui-body-container user">
                <div class="layui-form user_part">
                    <div class="layui-inline">
                        <label class="layui-form-label"><?php echo (L("_USER_FULL_NAME_")); ?></label>
                        ...
                        <div class="layui-input-inline">
                            <input type="text" name="title" id="username" lay-verify="title" autocomplete="off" placeholder="<?php echo (L("_USER_INPUT_NAME_")); ?>" class="layui-input">
                        </div>
                    </div>

                    <!--<div class="layui-inline">
                        <label class="layui-form-label"><?php echo (L("_USER_IDENTIFY_")); ?></label>
                        <div class="layui-input-inline">
                            <select name="modules" id="person-type" lay-verify="required" lay-search="">
                                <option value=""><?php echo (L("_SELECT_OR_SEARCH_")); ?></option>
                                <?php if(is_array($rolename_list)): foreach($rolename_list as $key=>$var): ?><option value="<?php echo ($var["ROLE_ID"]); ?>"><?php echo ($var["ROLE_NAME"]); ?></option><?php endforeach; endif; ?>
                            </select>
                        </div>
                    </div>-->
                    ...
                    <div class="layui-inline">
                        <div class="layui-input-block">
                            <button class="layui-btn search-btn"><?php echo (L("_GO_SEARCH_")); ?></button>
                        </div>
                    </div>
                    ...
                    <table class="layui-table">
                        ...
                        <thead>
                        <tr>
                            <th><?php echo (L("_USER_NO_")); ?></th>
                            <th><?php echo (L("_USER_NAME_")); ?></th>
                            <th><?php echo (L("_USER_FULL_NAME_")); ?></th>
                            <th><?php echo (L("_USER_IDENTIFY_")); ?></th>
                            <th><?php echo (L("_USER_PHONE_")); ?></th>
                            <th><?php echo (L("_USER_AUDIT_STATUS_")); ?></th>
                            <th><?php echo (L("_OPERATING_")); ?></th>
                        </tr>
                        </thead>
                        <tbody>
                        <?php if(!empty($userInfo)): if(is_array($userInfo)): foreach($userInfo as $key=>$var): ?><tr>
                                <td><?php echo ($var["USER_ID"]); ?></td>
                                <td><?php echo ($var["USER_LOGINAME"]); ?></td>
                                <td><?php echo ($var["USER_NAME"]); ?></td>
                                <td><?php echo ($var["ROLE_NAME"]); ?></td>
                                <td><?php echo ($var["CELLPHONE"]); ?></td>
                                <td>
                                    <?php echo (L("_USER_START_STATUS_")); ?>
                                </td>
                                <td data-id="<?php echo ($var["USER_ID"]); ?>">
                                    <button class="layui-btn layui-btn-radius edit" role-id=<?php echo ($var["ROLE_ID"]); ?>><?php echo (L("_EDIT_")); ?></button>
                                    <button class="layui-btn layui-btn-warm layui-btn-radius edit-pw"><?php echo (L("_USER_EDIT_PASSWORD_")); ?></button>
                                    <!--<?php if($var["AUTH"] == '1'): ?>-->
                                        <!--<a href="/index.php/Home/User/user_list_auth?aid=<?php echo ($var["USER_ID"]); ?>&rid=<?php echo ($var["ROLE_ID"]); ?>" class="layui-btn layui-btn-danger layui-btn-radius auth">
                                            <?php echo (L("_USER_AUDIT_MANAGE_")); ?>
                                        </a>-->
                                    <!--<?php endif; ?>-->
                                    <!--<button class="layui-btn layui-btn-radius layui-btn-danger del">删除</button>-->
                                </td>
...
<!--按每个页面引入-->
<script src="/Public/static/js/user/user_list.js?data=171208"></script>
</html>